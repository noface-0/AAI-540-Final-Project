...
class ActorSAC(nn.Module):
    def __init__(self, state_dim: int, action_dim: int, hidden_dims: List[int]):
        super().__init__()
        self.net = build_mlp([state_dim, *hidden_dims, action_dim * 2])  # Mean and log_std
    ...
    def sample_action(self, state: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
...
class AgentSAC(AgentBase):
    def __init__(self, state_dim: int, action_dim: int, hidden_dims: List[int], args: Config = Config(), gpu_id: int = None):
        # Ensure hidden_dims is a list or tuple of integers
        if not isinstance(hidden_dims, (list, tuple)):
            hidden_dims = [hidden_dims]
        
        self.act_class = ActorSAC
        self.cri_class = CriticSAC
        super().__init__(hidden_dims, state_dim, action_dim, gpu_id, args)
        ...
        self.target_critic1 = CriticSAC(state_dim, action_dim, hidden_dims)
        self.target_critic2 = CriticSAC(state_dim, action_dim, hidden_dims)

        self.target_critic1.load_state_dict(self.critic1.state_dict())
        self.target_critic2.load_state_dict(self.critic2.state_dict())

        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=args.lr_actor)
        self.critic1_optimizer = optim.Adam(self.critic1.parameters(), lr=args.lr_critic)
        self.critic2_optimizer = optim.Adam(self.critic2.parameters(), lr=args.lr_critic)
    ...
    def update(self, state, action, reward, next_state, done):
        with torch.no_grad():
            next_action, next_log_prob, _ = self.actor.sample_action(next_state)
            target_q1_next, target_q2_next = self.target_critic1(next_state, next_action), self.target_critic2(next_state, next_action)
            min_target_q_next = torch.min(target_q1_next, target_q2_next) - self.alpha * next_log_prob
        ...
        # Optional: Update Alpha (Entropy Coefficient)
        # This part is optional and depends on whether you're automatically tuning alpha.
        # If you are, you would include an optimization step for alpha here based on the desired entropy target.
